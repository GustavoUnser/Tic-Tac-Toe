...
import { UseBoardResults } from "./interface"
...
const useBoard = (): UseBoardResults => {
    ...
    const checkVictory = () => {
        return(
            allEqual(board[0]) ||
            allEqual(board[1]) ||  //Horizontais
            allEqual(board[2]) ||

            allEqual([board[0][0], board[1][0], board[2][0]]) ||
            allEqual([board[0][1], board[1][1], board[2][1]]) || //Verticais
            allEqual([board[0][2], board[1][2], board[2][2]]) ||

            allEqual([board[0][0], board[1][1], board[2][2]]) ||  //Diagonais
